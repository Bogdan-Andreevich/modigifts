...
class ControllerExtensionPaymentOPGooglePay extends Controller {
	...
	public function op_googlepay_form() {
		...
		if (!empty($order_info)) {
			...
			$filedate   = 'oceanpayment_googlepay_'.date('Y-m-d');
			$postdate = date('Y-m-d H:i:s');
			$newfile    = fopen( DIR_LOGS . $filedate . ".log", "a+" );
			...
			$post_log = $post_log . "*************************************\r\n";
			$post_log = $post_log.file_get_contents( DIR_LOGS . $filedate . ".log");
			$filename   = fopen( DIR_LOGS . $filedate . ".log", "r+" );
	...
	public function returnLog($logType){
		...
		$return_log = $return_log.file_get_contents( DIR_LOGS . $filedate . ".log");
		$filename   = fopen( DIR_LOGS . $filedate . ".log", "r+" );
