...
class ControllerExtensionModuleWbcategoryTab extends Controller {
	public function index($setting) {
		$this->load->language('extension/module/wbcategory_tab');
		...
		if (!empty($setting['category'])) {
			...
			foreach ($categories as $category_id) {
				...
				$datainfo['image'] = $this->model_tool_image->resize($category['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_height'));
				...
				if (array_filter($category_info) != 0) {
		...
		if ($data['categories']) {
			return $this->load->view('extension/module/wbcategory_tab', $data);
		}
	}
}