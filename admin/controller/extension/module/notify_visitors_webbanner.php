...
class ControllerExtensionModulenotifyvisitorswebbanner extends Controller {
    ...
    public function index() {
        if (!isset($this->request->get['module_id'])) {
			...
			$this->response->redirect($this->url->link('extension/module/notify_visitors_webbanner','&user_token='.$this->session->data['user_token'].'&module_id='.$module_id));
		} else {
    ...
    private function addModule() {
		...
		$this->model_setting_module->addModule('notify_visitors_webbanner', self::DEFAULT_MODULE_SETTINGS);
	...
	protected function editModule($module_id) {
		...
		$this->load->language('extension/module/notify_visitors_webbanner');
			if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate()) {
				$this->request->post['name'] = 'NotifyVisitors webbanner';
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/notify_visitors_webbanner', 'user_token=' . $this->session->data['user_token'], true)
        );
		...
		$htmlOutput = $this->load->view('extension/module/notify_visitors_webbanner', $data);
		$this->response->setOutput($htmlOutput);
	}
	public function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/notify_visitors_webbanner')) {
    ...
    public function install() {
		...
		$this->model_setting_setting->editSetting('module_notify_visitors_webbanner', ['module_notify_visitors_webbanner_status'=>1]);
    }
 
    public function uninstall() {
		$this->load->model('setting/setting');
		$this->model_setting_setting->deleteSetting('module_notify_visitors_webbanner');
    }
}